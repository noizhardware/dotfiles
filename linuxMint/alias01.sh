...
## PATH
	...
	#unset PYTHONHOME
	export PATH="$PATH:/media/nff/RED/s/c/bu/bin"
...
## SYSTEM
	...
	alias tem='temp 2>/dev/null | grep "Package id 0"'
	alias gpu='nvidia-smi ; nvidia-smi | grep -i p8'
	...
	alias ipp='curl -s ifconfig.me && echo ""'
...
## PROGRAMS quick launch
	...
	alias noo='/media/nff/RED/s/c/noote/noote'
	alias goo='katrain'
	...
	alias mas='/media/nff/RED/s/c/masyu/masyu ; qq'
	alias ult='/media/nff/RED/s/c/ultimatic/ultimatic ; qq'
	yt3(){ #youtube to mp3
		...
		# /home/nff/Downloads/yt-dlp_linux/yt-dlp_linux "$1" --cookies-from-browser vivaldi -t mp3 -P "~/Downloads" --yes-playlist -o "%(playlist_index&{} - |)s%(title)s.%(ext)s"
		# /home/nff/Downloads/yt-dlp_linux/yt-dlp_linux "$1" -t mp3 -P "~/Downloads" --yes-playlist -o "%(playlist_index&{} - |)s%(title)s.%(ext)s" --cookies-from-browser vivaldi:/home/nff/.config/vivaldi
		/home/nff/Downloads/yt-dlp_linux/yt-dlp_linux "$1" -t mp3 -P "~/Downloads" --yes-playlist -o "%(playlist_index&{} - |)s%(title)s.%(ext)s"
	}
	...
	seaa(){ # usage: seaa [searchterm] ; eg; `seaa porcodio`
		sea "*.*" "$1"
	}
	sef(){ # search filenames ; usage: sef [filematch] [searchterm] ; eg; `sef "*.md" porcodio`
		find . -type f -name "$1" | grep --color -i "$2"
	}
	sefa(){ # search filenames ON ALL FILES ; usage: sefa [searchterm] ; eg; `sef "*.md" porcodio`
		sef "*.*" "$1"
	}
	...
	alias ard='arduino-cli board list'
	car(){ # Carla
		qpwgraph &
		pw-jack carla &
		disown -a # detaches both from the shell
	}

	alias aud='qpwgraph & disown -a'
	alias tim='/media/nff/RED/Work/git/dotfiles/util/tai-clock.sh'

## ZK0
	ZK0BIN=/media/nff/RED/s/c/zk0/bin/zk0
	ZETPATH=/media/nff/HOTdata/LOGS/zet
	alias zk='$ZK0BIN $ZETPATH'
	alias zkk='cd $ZETPATH'
	# alias tt='nano template.md'
	alias fle='nano $ZETPATH/fleeting.txt'
	alias nn='$ZETPATH/new.sh'
...
## opening folders in file manager window
	...
	alias doc='xdg-open /media/nff/RED/documents'
...
## opening files
	...
	alias ww0='xdg-open /media/nff/RED/MUCKE/focus/whitenoiseLow.mp3'
	alias ww1='xdg-open /media/nff/RED/MUCKE/focus/whitenoise.mp3'
	alias ww2='xdg-open /media/nff/RED/MUCKE/focus/whitenoiseHarsh.mp3'
	# alias wwv='xdg-open /media/nff/RED/MUCKE/focus/voicescramble.mp3'
	alias wwv='nohup celluloid --mpv-loop=inf /media/nff/RED/MUCKE/focus/voicescramble.mp3 >/dev/null 2>&1 & disown'
	...
	alias szg='xdg-open /media/nff/RED/documents/contabilita/acc_2026/suzgeld2026.ods'
